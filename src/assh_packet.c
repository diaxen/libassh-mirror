...
assh_scan_blob(const char *format, const uint8_t **blob, size_t *blob_len, ...)
{
  ...
  st->next = st->blob = *blob;
  st->blob_len = *blob_len;

  const uint8_t *s = NULL, *r = NULL;
  ...
  while (1)
    {
      switch (*format++)
        ...
        case 's':
          ...
          s = st->next;
          r = s + 4;
          ASSH_JMP_ON_ERR(assh_check_string(st->blob, st->blob_len,
                                            s, &st->next), err);
          break;
        ...
        case 'a': {
          ...
          s = st->next;
          uint_fast8_t id = strtoul(format, (char**)&format, 10);
          ASSH_JMP_ON_ERR(assh_check_asn1(st->blob, st->blob_len,
                                          s, &r, &st->next, id), err);
          break;
        }
        ...
        raw: {
          size_t l = strtoul(format, (char**)&format, 10);
          if (l == 0)
            l = va_arg(ap, size_t);
          r = s = st->next;
          ASSH_JMP_ON_ERR(assh_check_array(st->blob, st->blob_len,
                                           s, l, &st->next), err);
          break;
        }
        ...
        case 'B': {
          ...
          ASSH_JMP_ON_ERR(assh_bignum_size_of_data(bnfmt,
                            s, &l, NULL, &bits), err);
        ...
        case 'T': {
          ...
          ASSH_JMP_IF_TRUE(l != st->next - r,
                           ASSH_ERR_BAD_DATA, err);
          break;
        }
        ...
        case 'E': {
          size_t o = strtoul(format, (char**)&format, 10);
          assert(*format == ';');
          format++;
          size_t l = strtoul(format, (char**)&format, 10);
          assert(*format == ';');
          format++;
          ASSH_JMP_IF_TRUE(o + l > st->next - r ||
                           memcmp(format, r + o, l),
                           ASSH_ERR_BAD_DATA, err);
          format += l;
          break;
