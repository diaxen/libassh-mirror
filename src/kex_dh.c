                                              ...
                                              struct assh_packet_s *p)
{
  ...
  ASSH_ERR_GTO(assh_kex_server_hash1(s, l, hash_ctx, &pout, &slen, &hk), err_hash);
  ...
  ASSH_ERR_GTO(assh_bignum_bytecode(c, bytecode, "MMMMMNssTTTTTT",
                   gr->generator, gr->prime, e_str, f_str, secret, &pv->fn,
