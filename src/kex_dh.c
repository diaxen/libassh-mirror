...
static assh_error_t assh_kex_dh_send_expmod(struct assh_session_s *s)
{
  ...
  ASSH_BIGNUM_ALLOC(s->ctx, gn, gr->size, ASSH_ERRSV_DISCONNECT, err_);
  ASSH_BIGNUM_ALLOC(s->ctx, pn, gr->size, ASSH_ERRSV_DISCONNECT, err_gn);
					      ...
					      struct assh_packet_s *p)
{
  ...
  ASSH_CHK_RET(msg != SSH_MSG_KEX_DH_REPLY, ASSH_ERR_PROTOCOL
	       | ASSH_ERRSV_DISCONNECT);
  ...
  /* diffie hellman stuff */
  ASSH_BIGNUM_ALLOC(s->ctx, fn, gr->size, ASSH_ERRSV_DISCONNECT, err_);

  ASSH_ERR_GTO(assh_bignum_from_mpint(fn, NULL, f_str)
	       | ASSH_ERRSV_DISCONNECT, err_fn);

  ASSH_BIGNUM_ALLOC(s->ctx, gn, gr->size, ASSH_ERRSV_DISCONNECT, err_fn);
  ASSH_BIGNUM_ALLOC(s->ctx, pn, gr->size, ASSH_ERRSV_DISCONNECT, err_gn);
  ...
  ASSH_BIGNUM_ALLOC(s->ctx, kn, gr->size, ASSH_ERRSV_DISCONNECT, err_pn);
  ...
  assh_hash_bytes_as_string(&sha1, &assh_sha1_update, (const uint8_t*)ASSH_IDENT,
			    sizeof(ASSH_IDENT) /* \r\n\0 */ - 3);
  assh_hash_bytes_as_string(&sha1, &assh_sha1_update, s->ident_str, s->ident_len);
  ...
  ASSH_ERR_GTO(sign_algo->f_verify(s->ctx, pv->host_key, 1, sign_ptrs, sign_sizes,
                 h_str + 4, end - h_str - 4, &sign_ok)
	       | ASSH_ERRSV_DISCONNECT, err_kn);

  ASSH_CHK_GTO(!sign_ok, ASSH_ERR_HOSTKEY_SIGNATURE
	       | ASSH_ERRSV_DISCONNECT, err_kn);

  /* setup new keys */
  ASSH_ERR_GTO(assh_kex_new_keys(s, &assh_hash_sha1, ex_hash, kn)
	       | ASSH_ERRSV_DISCONNECT, err_kn);
                                              ...
                                              struct assh_packet_s *p)
{
  ...
  ASSH_CHK_RET(msg != SSH_MSG_KEX_DH_REQUEST,
  ...
  ASSH_CHK_GTO(host_key == NULL, ASSH_ERR_MISSING_KEY | ASSH_ERRSV_DISCONNECT, err_);
  ...
  ASSH_BIGNUM_ALLOC(s->ctx, en, gr->size, ASSH_ERRSV_DISCONNECT, err_);
  ASSH_ERR_GTO(assh_bignum_from_mpint(en, NULL, e_str)
	       | ASSH_ERRSV_DISCONNECT, err_en);

  ASSH_BIGNUM_ALLOC(s->ctx, xn, gr->size, ASSH_ERRSV_DISCONNECT, err_en);
  ASSH_BIGNUM_ALLOC(s->ctx, fn, gr->size, ASSH_ERRSV_DISCONNECT, err_xn);
  ASSH_BIGNUM_ALLOC(s->ctx, gn, gr->size, ASSH_ERRSV_DISCONNECT, err_fn);
  ASSH_BIGNUM_ALLOC(s->ctx, pn, gr->size, ASSH_ERRSV_DISCONNECT, err_gn);

  ASSH_ERR_GTO(assh_bignum_rand(s->ctx, xn, ASSH_PRNG_QUALITY_EPHEMERAL_KEY)
	       | ASSH_ERRSV_DISCONNECT, err_pn);
  ...
  ASSH_ERR_GTO(host_key->f_output(s->ctx, host_key, NULL, &ks_len,
	         ASSH_KEY_FMT_PUB_RFC4253_6_6)
	       | ASSH_ERRSV_DISCONNECT, err_pn);

  ASSH_ERR_GTO(sign_algo->f_generate(s->ctx, host_key, 0, NULL, NULL, NULL, &sign_len)
	       | ASSH_ERRSV_DISCONNECT, err_pn);

  ASSH_ERR_GTO(assh_packet_alloc(s->ctx, SSH_MSG_KEX_DH_REPLY,
		(4 + ks_len) + assh_bignum_mpint_size(fn) + (4 + sign_len), &pout)
	       | ASSH_ERRSV_DISCONNECT, err_pn);

  /* append public host key to packet. */
  uint8_t *ks_str;
  ASSH_ASSERT(assh_packet_add_string(pout, ks_len, &ks_str));
  ASSH_ERR_GTO(host_key->f_output(s->ctx, host_key, ks_str, &ks_len,
		ASSH_KEY_FMT_PUB_RFC4253_6_6)
	       | ASSH_ERRSV_DISCONNECT, err_p);
  ...
  ASSH_ERR_GTO(assh_packet_add_mpint(pout, fn)
	       | ASSH_ERRSV_DISCONNECT, err_p);
  ...
  assh_hash_string(&sha1, &assh_sha1_update, f_str);
  ASSH_ERR_GTO(assh_hash_bignum(&sha1, &assh_sha1_update, pv->kn)
	       | ASSH_ERRSV_DISCONNECT, err_p);
  ...
  ASSH_ERR_GTO(sign_algo->f_generate(s->ctx, host_key, 1,
		 sign_ptrs, sign_sizes, sign, &sign_len)
	       | ASSH_ERRSV_DISCONNECT, err_p);
  assh_packet_shrink_string(pout, sign, sign_len);

  /* setup new symmetric keys */
  ASSH_ERR_GTO(assh_kex_new_keys(s, &assh_hash_sha1, ex_hash, pv->kn)
	       | ASSH_ERRSV_DISCONNECT, err_p);
